...
validate_ip() {
    ...
    # 1. ЖЕСТКАЯ ОЧИСТКА: Удаляем все пробелы, табуляции и невидимые символы
    ip=$(echo "$ip" | tr -d '[:space:]')

    # 2. Проверяем общий формат: цифры.цифры.цифры.цифры
    if [[ "$ip" =~ ^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
        # Разбиваем на массив по точке
        ...
        for octet in "${octets[@]}"; do
            # Конструкция $((10#$octet)) заставляет Bash считать число десятичным,
            # даже если оно начинается с 0 (например, 08), чтобы не было ошибки.
